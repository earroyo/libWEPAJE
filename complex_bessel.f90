Module Complex_Bessel
  ...
  PRIVATE
  PUBLIC  :: cbesh, cbesi, cbesj, cbesk, cbesy, cairy, cbiry, besseli, besselk, CGAMA, CITIKA, ITIKA 

  ! Oct 2011: replaced function gamln() with Fortran2008 built-in log_gamma()
  ! Elkin Arroyo Added log_gamma in the same file. g95 was not able to 
  ! find log_gamma as implicit function
...
! Added Back Calculation of Log_Gamma f95 was missing this intrinsic function
! Source of code below from
! 	****************************************
!		*           DISK TO ACCOMPANY          *
...
! http://people.sc.fsu.edu/~jburkardt/f_src/special_functions/special_functions.f90
        ...
        REAL(dp):: G0,GI1,GR1,SR,SI,T,TH,TH1,TH2,X0,X1,Y1,Z1,Z2,PI
        INTEGER :: K,J,NA
!        REAL(dp), DIMENSION (10) :: A
   REAL (dp), SAVE, DIMENSION (10) :: A = (/ &
...
!A(1) = 8.333333333333333D-02
!A(2) =-2.777777777777778D-03
!A(3) = 7.936507936507937D-04
!A(4) =-5.952380952380952D-04
!A(5) = 8.417508417508418D-04
!A(6) =-1.917526917526918D-03
!A(7) = 6.410256410256410D-03
!A(8) =-2.955065359477124D-02
!A(9) = 1.796443723688307D-01
!A(10)=-1.39243221690590D+00
...
!-----------------------------------------
FUNCTION log_gamma(a) result(lngamma)
!----------------------------------------
REAL (dp) :: lngamma
REAL (dp) :: a
REAL (dp) :: GR,GI
...
! Matlab like Sintaxis 
!----------------------------------------------------------------
function besselk(n,x) result(cx)
!----------------------------------------------------------------
implicit none
! INTEGER, PARAMETER  :: dp = SELECTED_REAL_KIND(15, 307)
integer     :: numzero, ierr, n
complex(dp) :: x
! real(dp)    :: fnu
complex(dp), dimension(0:0) :: cy
complex(dp) :: cx

if(n.eq.0) then
   call cbesk(z=x, fnu=0.0_DP, kode=1, n=1, cy=cy,  nz=numzero, ierr=ierr)
   cx =cy(0)
!   besselk=cy
else
   call cbesk(z=x, fnu=1.0_DP, kode=1, n=1, cy=cy,  nz=numzero, ierr=ierr)
   cx =cy(0)
!   besselk=cy
end if
return
...
!----------------------------------------------------------------
function besseli(n,x) result(cx)
!----------------------------------------------------------------
implicit none
! INTEGER, PARAMETER  :: dp = SELECTED_REAL_KIND(15, 307)
integer     :: numzero, ierr, n
...
complex(dp), dimension(0:0) :: cy
complex(dp) :: cx
! complex(dp), dimension(0:0) :: besseli

if(n.eq.0) then
   ! write(*,*) 'Before Calling cbesi(z=x, fnu=0.0_DP, kode=1, n=1, cy=cy,  nz=numzero, ierr=ierr)'     
   call cbesi(z=x, fnu=0.0_DP, kode=1, n=1, cy=cy,  nz=numzero, ierr=ierr)
   cx =cy(0)
!   besseli=cy
else
   call cbesi(z=x, fnu=1.0_DP, kode=1, n=1, cy=cy,  nz=numzero, ierr=ierr)
   cx =cy(0)
!   besseli=cy
end if
return
...
REAL(dp) :: RC2,TW,RS,B2,B1,E0,RC1,R,X2,EL,PI
INTEGER  :: K
        REAL(dp), save, dimension (10) :: A = (/ &
        .625D0,1.0078125D0,2.5927734375D0,     & 
        9.1868591308594D0, 4.1567974090576D+1, & 
        2.2919635891914D+2, 1.491504060477D+3, &
        1.1192354495579D+4, 9.515939374212D+4, &
        9.0412425769041D+5/)
...
!-------------------------------------------------------------------
        SUBROUTINE CITIKA(X,TI,TK)
!-------------------------------------------------------------------
!  Modified function so now it works with complex arguments
!  follows Matlab convention of comparing real only on 
!  complex(20,24) < 21 -> true. Ignores imaginary part
!  not certain about the validity of the function but it seems
!  to produce same as its Matlab counter part function.
!  One wonders how can integrate from -2+i to 2+i what is the meaning
!  of this integral?
...
!C      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
        ...
        COMPLEX (dp), INTENT(out):: TK
        
        COMPLEX (dp) :: R,X2, RC1, RC2, E0, B1, B2, TW
        REAL (dp)    ::  RS
        REAL(dp) :: EL,PI
        INTEGER  :: K
        ...
        IF (REAL(X).EQ.0.0D0) THEN
           TI=0.0D0
           TK=0.0D0
           RETURN
        ELSE IF (REAL(X).LT.20.0D0) THEN
           X2=X*X
           TI=1.0D0
           R=1.0D0
           DO 10 K=1,50
              R=.25D0*R*(2*K-1.0D0)/(2*K+1.0D0)/(K*K)*X2
              TI=TI+R
              IF (ABS(R/TI).LT.1.0D-12) GO TO 15
10         CONTINUE
15         TI=TI*X
        ELSE
           TI=1.0D0
...
20            TI=TI+A(K)*R
        ...
        IF (REAL(X).LT.12.0D0) THEN
           ...
           DO 25 K=1,50
              ...
              B2=B2+R*RS
              TK=B1+B2
              IF (ABS((TK-TW)/TK).LT.1.0D-12) GO TO 30
25            TW=TK
30         TK=TK*X
        ELSE
           TK=1.0D0
